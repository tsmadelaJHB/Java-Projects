...
import static za.co.wethinkcode.weshare.claim.ClaimExpenseController.*;
import static za.co.wethinkcode.weshare.claim.ClaimsApiController.API_PATH;

public class WeShareServer {
    private static final int DEFAULT_PORT = 9070;
    ...
    private static void claimRoutes() {
        path(PATH, () -> get(ClaimExpenseController::renderClaimExpensePage));
        path(API_PATH, () -> post(ClaimsApiController::create));
    }

    private static void expenseRoutes() {

        get(CaptureExpenseController.NEW_EXPENSE_PATH, CaptureExpenseController::createExpense);
        post(CaptureExpenseController.NEW_EXPENSE_PATH, CaptureExpenseController::saveExpenseData);
